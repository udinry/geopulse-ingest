...
- `src/alerts/dispatch.ts`'s push payload (`aps.alert.title`/`.body`) is the one piece of user-facing copy this repo generates — everything else is JSON data, not display text. It's still subject to the banned betting/trading vocabulary rule from the app repo's `docs/ARCHITECTURE.md` (outlook section) even though `../GeoPulse/Scripts/lint-copy.sh` only scans the app repo's Swift files; check it by eye if you touch this string.
